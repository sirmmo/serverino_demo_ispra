...
@app.post('/data_2.geojson')
async def filter_data_2(body: str):
    shp = json.loads(body)
    is_valid = dPolygon(**shp)
    poly = shape(shp)
    geo_osservazioni['geom2'] = geo_osservazioni['geometry'].intersection(poly)
    #ret = geo_osservazioni[mask]
    ret = geo_osservazioni[~geo_osservazioni['geom2'].is_empty]
    ret['geom2_serialized'] = ret['geom2'].apply(lambda x: json.dumps(x.__geo_interface__))
    print(ret)
